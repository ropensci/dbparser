...
test_that(
  ...
  code = {
    expect_match(
      as.character(parse_drug_groups()[1][["group"]][1]),
      "approved"
    )
...
test_that(
  ...
  code = {
    expect_match(as.character(
      parse_drug_articles()[["pubmed-id"]][1]
    ), "16466327")
...
test_that(
  ...
  code = {
    expect_match(
      as.character(
        parse_drug_categories()[["mesh-id"]][[1]]
      ),
      "D000602"
    )
...
test_that(
  ...
  code = {
    expect_match(
      as.character(
        parse_drug_atc_codes()[["atc_code"]][[1]]
      ),
      "B01AE06"
    )
...
test_that(
  ...
  code = {
    expect_equal(
      nrow(parse_snp_adverse_reactions()),
      0
    )
